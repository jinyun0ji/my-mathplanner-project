        ...
        <div className="w-full xl:w-80 flex-shrink-0 bg-white p-4 rounded-xl shadow-md space-y-4">
            <div className="flex justify-between items-center border-b pb-2">
                ...
                <button
                    ...
                    className="text-[#455fab] hover:text-[#334a91] text-sm font-medium flex items-center"
                >
            ...
            <select
                ...
                className="w-full p-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-700 focus:ring-[#455fab] focus:border-[#455fab]"
            >
            ...
            {selectedClass && (
                <div className={`border p-3 rounded-lg space-y-2 ${isSelectedClassClosed ? 'bg-gray-100 border-gray-300' : 'bg-[#f1f4ff] border-[#eef2ff]'}`}>
                    <p className={`text-sm font-semibold ${isSelectedClassClosed ? 'text-gray-700' : 'text-[#334a91]'}`}>
                        {selectedClassGrade || '학년 미정'} | {formatClassScheduleKo(selectedClass) || '시간 미정'}
                    </p>
                    <p className={`text-xs ${isSelectedClassClosed ? 'text-gray-600' : 'text-[#455fab]'}`}>
                    ...
                    {showEditButton && (
                        <button
                            ...
                            className={`text-xs font-medium flex items-center ${isSelectedClassClosed ? 'text-gray-500 hover:text-gray-700' : 'text-[#455fab] hover:text-[#334a91]'}`}
                        >
