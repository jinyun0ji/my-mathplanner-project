...
export const DashboardTab = ({ student, myClasses, attendanceLogs, clinicLogs, homeworkStats, notices, setActiveTab }) => {
    ...
    return (
        <div className="space-y-6 pb-6 animate-fade-in-up">
            ...
            <div><h3 className="text-lg font-bold text-gray-800 mb-3 px-1 flex items-center"><Icon name="calendar" className="w-5 h-5 mr-2 text-brand-main" />오늘의 일정</h3>{todayClasses.length === 0 && todayClinics.length === 0 ? (<div className="bg-white p-6 rounded-2xl border border-dashed border-gray-300 text-center text-gray-500 text-sm">오늘 예정된 수업이나 클리닉이 없어요. <br/>자율 학습을 해보는 건 어때요? 🔥</div>) : (<div className="space-y-3">{todayClasses.map(cls => (<div key={cls.id} className="bg-indigo-50 p-5 rounded-2xl border border-indigo-100 flex justify-between items-center"><div><span className="text-xs font-bold text-indigo-600 bg-white px-2 py-0.5 rounded border border-indigo-200 mb-2 inline-block">정규 수업</span><h4 className="font-bold text-indigo-900 text-lg">{cls.name}</h4><p className="text-sm text-indigo-700 mt-0.5">{cls.schedule.time} | 채수용 선생님</p></div><Icon name="chevronRight" className="text-indigo-400" /></div>))}{todayClinics.map(clinic => (<div key={clinic.id} className="bg-teal-50 p-5 rounded-2xl border border-teal-100 flex justify-between items-center"><div><span className="text-xs font-bold text-teal-600 bg-white px-2 py-0.5 rounded border border-teal-200 mb-2 inline-block">클리닉</span><h4 className="font-bold text-teal-900 text-lg">학습 클리닉</h4><p className="text-sm text-teal-700 mt-0.5">{clinic.checkIn} 입실 예정</p></div></div>))}</div>)}</div>
            ...
            <div><h3 className="text-lg font-bold text-gray-800 mb-3 px-1">📢 최근 소식</h3><div className="bg-white rounded-2xl shadow-sm border border-gray-200 divide-y divide-gray-100">{notices.slice(0, 3).map(notice => (<div key={notice.id} onClick={() => setActiveTab('menu')} className="p-4 flex justify-between items-center cursor-pointer active:bg-gray-50"><div className="flex-1 min-w-0 mr-4"><div className="flex items-center gap-2 mb-1">{notice.isPinned && <span className="text-[10px] bg-brand-red text-white px-1 rounded">필독</span>}<h4 className="text-sm font-bold text-gray-900 truncate">{notice.title}</h4></div><p className="text-xs text-gray-400">{notice.date}</p></div><Icon name="chevronRight" className="w-4 h-4 text-gray-300" /></div>))}{notices.length === 0 && (<div className="p-4 text-center text-gray-500 text-sm">새로운 공지사항이 없습니다.</div>)}</div></div>
        </div>
    );
};
        ...
        <div className="grid grid-cols-1 gap-4">
            {myClasses.map(cls => (
                <div key={cls.id} onClick={() => setSelectedClassId(cls.id)} className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm flex items-center justify-between cursor-pointer active:scale-[0.98] transition-transform">
                    <div className="flex gap-4 items-center"><div className="bg-brand-light/20 w-14 h-14 rounded-2xl flex items-center justify-center text-brand-main font-bold text-xl shrink-0">{cls.name.charAt(0)}</div><div><h4 className="font-bold text-gray-900 text-lg mb-1">{cls.name}</h4><p className="text-xs text-gray-500 font-medium bg-gray-100 px-2 py-1 rounded-md inline-flex items-center gap-1"><Icon name="users" className="w-3 h-3" /> 채수용 선생님</p><p className="text-xs text-gray-400 mt-1 ml-0.5">{cls.schedule.days.join(', ')} {cls.schedule.time}</p></div></div><div className="text-gray-300"><Icon name="chevronRight" className="w-6 h-6" /></div>
                </div>
            ))}
        </div>
    </div>
...
export const ScheduleTab = ({ 
    ...
    const renderSchedules = () => {
        ...
        const dailyClasses = myClasses.filter(cls => cls.schedule.days.includes(dayOfWeek)).map(cls => ({ id: `math-${cls.id}`, type: 'math', name: cls.name, teacher: '채수용', time: cls.schedule.time, scheduleId: cls.id }));
        ...
        return (
            <div className="grid grid-cols-1 gap-4">{allSchedules.map((item) => {
                ...
                return (<div key={item.id} className={`relative pl-6 border-l-2 py-2 ml-2 ${borderColor}`}><div className={`absolute -left-[9px] top-3 w-4 h-4 rounded-full ring-4 ring-white ${dotColor}`}></div><div onClick={(e) => item.type === 'external' ? handleEditClick(e, item) : null} className={`bg-white p-5 rounded-2xl shadow-sm border border-brand-gray/30 relative group h-full flex flex-col justify-between transition-all hover:shadow-md ${item.type === 'external' ? 'cursor-pointer hover:border-brand-main/50' : ''}`}><div><div className="flex justify-between mb-2"><span className={`text-xs font-bold px-2 py-1 rounded ${typeClass}`}>{typeLabel}</span><span className="text-xs text-brand-gray font-medium">{item.time}</span></div><h4 className="font-bold text-brand-black text-lg mb-2">{item.name}</h4></div><div className="flex justify-between items-end">{item.type === 'math' ? (<><p className="text-sm text-brand-gray flex items-center gap-1"><Icon name="users" className="w-4 h-4" /> 채수용 선생님</p>{log && (<span className={`text-xs font-bold px-2 py-1 rounded ${log.status === '출석' ? 'bg-green-100 text-green-700' : log.status === '지각' ? 'bg-yellow-100 text-yellow-700' : 'bg-brand-red/10 text-brand-red'}`}>{log.status}</span>)}</>) : item.type === 'clinic' ? (<><p className="text-sm text-brand-gray flex items-center gap-1"><Icon name="user" className="w-4 h-4" /> {item.teacher}</p><span className={`text-xs font-bold px-2 py-1 rounded ${item.status === '완료' ? 'bg-teal-100 text-teal-700' : 'bg-teal-50 text-teal-600 border border-teal-200'}`}>{item.status}</span></>) : (<div className="w-full flex justify-end gap-3"><span className="text-xs text-brand-main opacity-0 group-hover:opacity-100 transition-opacity">수정</span><button onClick={(e) => handleDeleteClick(e, item)} className="text-xs text-brand-gray hover:text-brand-red underline">삭제</button></div>)}</div></div></div>);
            })}</div>
        );
    };
        ...
        <div className="space-y-4">
            ...
            myGradeComparison.map((item, idx) => (
                <div key={idx} onClick={() => toggleTestDetails(item.testId)} className={`bg-white p-5 rounded-2xl shadow-sm border border-gray-100 cursor-pointer ${selectedTestId === item.testId ? 'ring-2 ring-indigo-500' : ''}`}>
                    ...
                    <div className="flex justify-between items-center mb-3 bg-gray-50 px-3 py-2 rounded-lg"><span className="text-xs font-bold text-gray-500">나의 정답률</span><div className="flex items-center gap-2"><div className="w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden"><div className={`h-full rounded-full ${(item.accuracy || 0) >= 90 ? 'bg-green-500' : (item.accuracy || 0) >= 70 ? 'bg-brand-main' : 'bg-red-500'}`} style={{ width: `${item.accuracy || 0}%` }}></div></div><span className={`text-xs font-bold ${(item.accuracy || 0) >= 90 ? 'text-green-600' : (item.accuracy || 0) >= 70 ? 'text-brand-main' : 'text-red-500'}`}>{item.accuracy || 0}%</span></div></div>
