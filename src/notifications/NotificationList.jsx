        ...
        <div className="fixed inset-0 z-[60] overflow-hidden pointer-events-none">
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-brand-bg custom-scrollbar">
                    {isLoading ? (
                        <div className="flex items-center justify-center h-32 text-sm text-brand-gray">알림을 불러오는 중...</div>
                    ) : sortedNotifications.length > 0 ? sortedNotifications.map((notification) => {
                        const createdAt = notification.createdAt;
                        const isUnread = !notification.readAt && (!lastReadAt || (createdAt?.toMillis && lastReadAt?.toMillis && createdAt.toMillis() > lastReadAt.toMillis()));
                        return (
                            <button
                                key={notification.id}
                                onClick={() => onNotificationClick(notification)}
                                className={`w-full text-left bg-white p-4 rounded-2xl shadow-sm border transition-all hover:shadow-md group ${isUnread ? 'border-brand-main/40 bg-brand-light/10' : 'border-brand-gray/20'}`}
                            >
