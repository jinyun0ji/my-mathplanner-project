...
const ScheduleTab = ({ myClasses, externalSchedules, attendanceLogs, studentId, onSaveExternalSchedule, onDeleteExternalSchedule }) => {
    ...
    const renderScheduleList = () => {
        ...
        const dailyClasses = myClasses.filter(cls => cls.schedule.days.includes(dayOfWeek)).map(cls => ({
            id: `math-${cls.id}`, type: 'math', name: cls.name, 
            teacher: '채수용', // ✅ 강사명 고정
            time: cls.schedule.time, scheduleId: cls.id
        }));
        ...
        <div className="pb-20 relative">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                    {/* ✅ [수정] 일정 추가 버튼 높이(py-0, h-[32px])를 주간/월간 버튼과 정확히 맞춤 */}
                    <button 
                        onClick={() => setIsScheduleModalOpen(true)}
                        className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-0 h-[32px] rounded-xl text-xs font-bold flex items-center gap-1 shadow-md transition-all active:scale-95"
                    >
