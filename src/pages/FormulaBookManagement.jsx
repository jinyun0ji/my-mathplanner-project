...
export default function FormulaBookManagement() {
  ...
  return (
    <div className="space-y-6 p-4 md:p-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><h1 className="text-2xl font-bold text-gray-900">공식집 관리</h1><p className="mt-1 text-sm text-gray-500">과목과 개념을 등록하고 QR 진입 링크를 관리합니다.</p></div><div className="flex gap-2"><button type="button" onClick={() => { setEditingSubjectId(''); setSubjectForm(emptySubject); }} className="rounded-lg bg-[#455fab] px-4 py-2 text-sm font-bold text-white">과목 추가</button><button type="button" onClick={() => { setEditingConceptId(''); setConceptForm(emptyConcept); }} className="rounded-lg bg-[#455fab] px-4 py-2 text-sm font-bold text-white">개념 추가</button></div></div>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <section className="rounded-2xl border bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 divide-y">{visibleConcepts.map((concept) => <div key={concept.id} className="flex items-center gap-2 py-3 text-sm"><button type="button" className="min-w-0 flex-1 text-left" onClick={() => { setEditingConceptId(concept.id); setConceptForm({ ...emptyConcept, ...concept, tags: Array.isArray(concept.tags) ? concept.tags.join(', ') : '' }); }}><b>{concept.title}</b><span className="ml-2 text-gray-400">{subjectTitleMap[concept.subjectId] || '과목 없음'} · {concept.active === false ? '비활성' : '활성'} · {concept.order || 0}</span><p className="mt-1 truncate text-xs text-gray-500">{concept.summary}</p><p className="mt-1 truncate text-xs text-gray-400">{concept.youtubeEmbedUrl}</p></button><button type="button" onClick={() => navigator.clipboard.writeText(buildFormulaConceptQrUrl(concept.id)).then(() => setMessage('QR 링크를 복사했습니다.'))} className="rounded-lg border px-2 py-1 text-xs font-semibold text-[#455fab]">QR 링크 복사</button><button type="button" onClick={() => deactivateConcept(concept.id)} className="rounded-lg border px-2 py-1 text-xs font-semibold text-red-600">비활성화</button></div>)}</div>
        </section>
      </div>
    </div>
  );
